...
- 修复多轮补充条件误开新商品请求：`四件套` 后补充颜色和搭配要求时继续沿用床品上下文，不再误切换成家具。
- 继续收紧商品边界：手机只按手机/品牌/型号/预算/规格筛选，耳机不串到音箱，床架不串到床垫，明确无货时不展示跨品类商品卡。
- 保留无单位预算识别能力：`预算4000`、`四千`、`不超过300`、后续改成 `800` 都按预算处理，不再误判为尺码或型号。

### 验证

- 新增 30 组自然中文多轮咨询回归，覆盖服饰、数码、电器、家具、家居用品、个护美妆、食品饮料、宠物用品、母婴用品、文具办公、照明灯具和运动健身；生产环境通过 `30/30`。
- 已验证生产页面从输入框提交咨询、追问品牌、返回真实商品卡的完整链路，浏览器无 warning/error。

### 线上发布

- Vercel 生产部署：`dpl_EFk1UTY7iq3ARJLVDYwCTQgiF7gK`，已绑定 [retail.abobb.site](https://retail.abobb.site)。
- 生产前端继续通过 `WORKER_CHAT_URL` 调用 Cloudflare Worker，商品数据来自已导入的真实商品库。

## [2026-08-03] — 分类追问话术自然化

### 修复

- 修复“给我推荐一款手机”返回内部规则说明的问题，改为直接询问品牌/型号和预算。
- 为手机、数码、家具、家居用品、个护美妆、食品饮料、宠物用品、母婴用品、文具办公、照明灯具、玩具和运动健身补充符合用户语境的追问话术。
- 移除“数码先确认”“只核对”“不追问面积或安装条件”“只在某分类里筛选”等内部逻辑表述，用户只会看到下一步需要补充的信息。
- 收紧空间和安装条件追问，仅空调、冰箱、洗衣机、洗碗机会默认询问容量、摆放或安装限制；咖啡机、吸尘器等其他电器不再被泛化要求提供面积或安装条件。
- 优化无匹配回复，去掉 `性别=male`、跨分类策略等内部字段和实现说明，直接告诉用户暂时没有符合条件的商品。
- 修复结构化商品被文本召回 `LIMIT 120` 截断的问题：现在先按一级分类、商品类型、性别和预算扫描 D1，再进行文本和语义排序，避免“男士跑步鞋预算500”漏掉真实的 449 元男士鞋履商品。
- 补全商品知识入库：材质、工艺、规格、风格、适用空间、使用场景和搭配提示现在会进入 D1 描述与向量文本，不再只用商品名称和一段 feature 做检索。
- 描述入库改用规范化细分类目，避免原始抓取字段与商品名称冲突时污染检索；源库重复 ID 按商品主键幂等合并，当前远端实际商品数为 2731 条。
- 过滤商品库中对所有品类复用的泛化“安装条件/试穿”避坑文案，避免手机、服饰等商品展示与品类无关的提醒。

### 验证

- 修复多轮预算修改被误识别为服饰尺码的问题：`预算500` 后续改成 `预算800` 时不再生成“尺码500”。
- 修复“阅读照明”等补充条件被前端误判为新的独立商品请求的问题，补充条件会继承上一轮的品类和预算。

- 新增手机、空调、咖啡机、沙发、台灯、运动服和动感单车等分类追问回归用例。
- 回归测试会拦截内部规则词，避免同类机器式话术再次上线。

## [2026-08-03] — 商品库分类重构 + 预算识别增强

### 新增

- 基于真实商品库建立结构化商品 facets，覆盖服饰、数码、电器、家具、家居用品、个护美妆、食品、母婴、玩具、照明和运动健身等分类。
- 为商品补充商品类型、细分类目、品牌、性别、尺码和规格属性，推荐时改用结构化字段筛选。
- 为不同商品分类设置独立追问逻辑：手机等数码商品只追问型号、品牌、预算和规格，不再追问面积或安装条件。
- 未扫描到符合条件的真实商品时明确返回无匹配结果，不跨分类或编造替代商品。

### 优化

- 废弃旧的全局关键词商品分类和展示分类猜测，改为商品库驱动的分类与推荐链路。
- 修复手机壳被识别为手机、家电被误归入数码、服饰细分类目错配等问题。
- 预算识别支持 `预算4000`、`预算是4000`、`预算为4,000`、`4000以内` 和单独回复 `4000`，不再要求用户明确输入“元”。
- 无法确认商品价格时显示“价格以官网为准”，不再把型号数字误当作售价。

### 验证

- 完成 2746 条商品与 2746 条结构化 facets 对齐，未归类商品为 0。
- 已通过 Worker dry run、Nuxt 生产构建和预算解析回归验证。

### 线上发布

- 已完成线上 D1 结构迁移和商品重导入：源数据 2746 条，D1 去重后 2731 条，分类字段覆盖率 100%。
...
- 新增前端“新会话”按钮：用户想开始下一轮独立选品时，可以一键清空历史消息、当前推荐卡、右侧推荐摘要和输入草稿，避免上一轮预算、品类和场景继续影响下一次推荐。
- 新增 DeepSeek 卡片文案润色层：Cloudflare Worker 先锁定真实商品，Nuxt 代理再调用 DeepSeek 只优化 `chat_reply`、推荐理由、适合人群、避坑建议和下一步引导，让卡片更像真人顾问，而不是生硬模板。
- 新增商品展示品类推导：Worker 返回 `category`，前端代理同步兜底推导 `T恤/短袖`、`照明灯具`、`鞋履`、`裤装`、`家居香氛` 等展示品类，卡片不再把灯具误标成运动服饰。

### 优化

- 多轮上下文策略升级：当最新用户消息已经包含明确品类、预算、人群或场景时，Nuxt 代理只把最新一轮需求发给 Worker，防止上一轮“灯光 300 元”污染下一轮“男士半袖 100 元以内”。
- 预算解析改为“最后预算优先”：同一句或多轮拼接里出现多个金额时，Worker 使用最后一次明确预算作为当前购买约束，更符合用户追加修正的真实表达。
- 人群过滤补强：`男式`、`女式` 等真实商品字段进入男女款识别，减少“男士半袖”被女款、童装或其他品类抢走的概率。
- 气泡布局更稳：聊天气泡继续按文本长度自适应，同时增加长文本断行和移动端最大宽度控制，避免长句撑破布局。
- Worker 兜底话术去工程味：推荐理由不再出现“关键词硬推”这类内部解释，改为面向用户的品类、预算和场景说明。

### 修复

- 修复第二次提问仍沿用第一次预算的问题：复现场景“小卧室灯光 300 元”后再问“男士通勤半袖 100 元以内”，现在稳定返回 `MUJI 男式 天竺编织 圆领短袖T恤 CNY 78`，不会再按 300 元找商品。
- 修复“推荐半袖结果弹鞋子/错品类”的体验问题：显式短袖、鞋履、裤装、外套、包袋请求会先走硬品类过滤，再参与排序。
- 修复前端卡片真实字段被文案层污染的风险：DeepSeek 只能改文案字段，`name`、`brand`、`price_range`、`image`、`source_url`、`signature_specs` 始终由 Worker/D1 锁定。
- 修复卡片品类假默认值：以前 Nuxt 代理把所有 Worker 商品都标成“运动服饰”，现在灯具显示“照明灯具”，短袖显示“T恤/短袖”。

### 验证

- 已验证生产 Worker：`再给我推荐一款男士 通勤的日常半袖100元以内` 返回 `MUJI 男式 天竺编织 圆领短袖T恤 CNY 78`，品类为 `T恤/短袖`。
- 已验证生产 Vercel 代理：多轮历史包含 `灯光 预算300元` 时，再问 `男士 通勤 日常半袖100元以内`，返回文案只使用 100 元预算，无 300 元串联。
- 已验证生产页面：发送长文本后气泡正常换行、自适应宽度；卡片渲染真实图片、官网链接和人群标签；点击“新会话”后消息数恢复为 1，推荐卡清空。
- 已通过 `npm run build`、`npx wrangler deploy --dry-run`，并部署到 `https://retail.abobb.site` 与 `https://retail-ai-agent-worker.abobb-retail-ai-agent.workers.dev`。
